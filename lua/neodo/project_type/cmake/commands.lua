...
function M.create_profile(ctx)
    ...
    picker.pick('Select build type: ', { 'Debug', 'Release', 'RelWithDebInfo', 'MinSizeRel' }, function(build_type)
        local function create_profile(bc)
            ...
            if bc then
                if bc.cmake_params then
                    profile.cmake_params = profile.cmake_params .. ' ' .. bc.cmake_params
                end
                profile.name = profile.name .. '-' .. bc.name
                profile.env_vars = bc.env_vars or nil
                profile.build_args = bc.build_args or nil
                profile_key = profile_key .. '-' .. string.gsub(bc.name, '%s+', '-')
            end
        ...
        if cmake_project.build_configurations then
            picker.pick(
                ...
                function(bc_key)
                    local build_configuration = cmake_project.build_configurations[bc_key]
...
function M.build_all(ctx)
    ...
    if profile.build_args then
        args_string = profile.build_args
    end
    return 'cmake --build ' .. profile.build_dir .. ' -- ' .. args_string
end
...
function M.build_selected_target(ctx)
    ...
    return 'cmake --build '
        .. profile.build_dir
        .. ' --target '
        .. cmake_project.config.selected_target
        .. ' -- '
        .. args_string
end
...
function M.run_selected_target(ctx)
    ...
    return fs.join_path(cmake_profile.build_dir, target.paths[1])
end
