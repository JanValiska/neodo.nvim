...
local function load_config(project)
    ...
    fs.read(config_file, 438, function(err, data)
        if err then
            ...
            print(vim.inspect(project.code_models))
            load_conan()
        end
    end)
end
...
local function create_profile(_, project)
    vim.ui.input({ prompt = "Provide new profile name: ", default = "Debug", kind = 'neodo.input.center' }, function(input)
        local profile = {}
        profile.name = input
    ...
    settings.project_type.cmake = {
        ...
        commands = {
            ...
            run_selected_target = {
                ...
                cmd = function(_, project)
                    local target = get_targets(project)[project.config.selected_target]
                    return { type = "success", text = target.paths[1] }
                end,
                enabled = function(_, project)
                    return project.config.selected_target ~= nil and get_targets(project)[project.config.selected_target].type == "EXECUTABLE"
                end,
            },
            conan_install = {
                type = "terminal",
