...
function CodeModel:read_reply()
    ...
    if index ~= nil then
        self:load_model_file(index, function(index_model)
            self.model = index_model
            self:load_model_file(index_model.reply['codemodel-v2'].jsonFile, function(code_model)
                self.model.reply['codemodel-v2'].model = code_model
                for cindex , configuration in ipairs(index_model.reply['codemodel-v2'].model.configurations) do
                    for tindex, target in ipairs(configuration.targets) do
                        self:load_model_file(target.jsonFile, function(target_model)
                            self.model.reply['codemodel-v2'].model.configurations[cindex].targets[tindex].model = target_model
                        end)
                    end
                end
            end)
...
function CodeModel:get_targets()
    local targets = {}
    for _, target in ipairs(self.model.reply['codemodel-v2'].model.configurations[1].targets) do
        table.insert(targets, target.model.name)
    end
    return targets
end
